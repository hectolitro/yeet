...
func (r *registry) v2(resp http.ResponseWriter, req *http.Request) *regError {
	...
	// Override the host header to funnel all requests to the same set of
	...
	if isManifest(req) {
		return r.manifests.handle(resp, req, r.callbackHandler)
	}
	if isTags(req) {
...
func New(opts ...Option) http.Handler {
	r := &registry{
		...
		manifests: manifests{
			manifestHandler: &memManifestHandler{m: map[string]map[string]Manifest{}},
			log:             log.New(os.Stderr, "", log.LstdFlags),
		},
	}
	for _, o := range opts {
...
type CallbackHandler interface {
	OnImageReceived(repo, tag, manifest string) error
}
